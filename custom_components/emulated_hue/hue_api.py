...
def entity_to_json(config, entity, state):
    ...
    def format_int(state, field, max_val):
        """
        Ensure we return a valid value.
        Return 0 if light is off, or value is none
        """
        if not state[STATE_ON]:
            return 0

        val = state.get(field, 0)
        if val is None:
            # just incase state[field] is None, make sure we return 0
            return 0
    ...
    return {
        'state':
        {
            ...
            HUE_API_STATE_BRI:
                format_int(state, STATE_BRIGHTNESS, HUE_API_STATE_BRI_MAX),
            HUE_API_STATE_HUE:
                format_int(state, STATE_HUE, HUE_API_STATE_HUE_MAX),
            HUE_API_STATE_SAT:
                format_int(state, STATE_SATURATION, HUE_API_STATE_SAT_MAX),
            'reachable': True
        },
        'type': 'Dimmable light',
